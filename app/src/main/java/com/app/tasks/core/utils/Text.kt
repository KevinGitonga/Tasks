...
interface Text : Parcelable {
     ...
     */
    @Composable
    operator fun invoke(): String = toString(LocalContext.current.resources)
 ...
 */
@Parcelize
private data class TextConcatenation(
    private val args: List<Text>,
) : Text {
    override fun invoke(res: Resources): CharSequence = args.joinToString(separator = "") { it.invoke(res) }
}
...
private data class ResArgsText(
    ...
    override fun invoke(res: Resources): String = res.getString(id, *convertArgs(res, args).toTypedArray())
...
private data class PluralsText(
    ...
    override fun invoke(res: Resources): String = res.getQuantityString(id, quantity, *convertArgs(res, args).toTypedArray())

    override fun toString(): String = "PluralsText(id=$id, quantity=$quantity, args=${args.contentToString()})"
}
...
private fun convertArgs(
    res: Resources,
    args: List<Any>,
): List<Any> = args.map { if (it is Text) it.invoke(res) else it }

/**
 * Implementation of [Text] backed by a raw string. For use with server responses.
 */
@Parcelize
private data class StringText(
